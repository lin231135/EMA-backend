...
SELECT 1, 1, s.id, 7,  'academia', 'programada', NOW(), 'Clase HOY (Diego)'
FROM s_hoy_1 s
UNION ALL
SELECT 2, 2, s.id, 9,  'academia', 'programada', NOW(), 'Clase HOY (Valeria)'
FROM s_hoy_2 s
UNION ALL
SELECT 3, 3, s.id, 10, 'academia', 'programada', NOW(), 'Clase HOY (Santiago)'
FROM s_hoy_3 s
UNION ALL
SELECT 17, 1, s.id, 7, 'academia', 'programada', NOW(), 'Clase mañana (Juan)'
FROM s_manana s
UNION ALL
SELECT 1, 2, s.id, 9, 'academia', 'programada', NOW(), 'Clase en 2 días (Diego)'
FROM s_pasado s
;
